//Code was found from the following source:
...
#ifndef REGULARSAMPLESORT_OMP_REGSAMPLESORT_H
#define REGULARSAMPLESORT_OMP_REGSAMPLESORT_H
...
void calc_partition_borders(int array[], int start, int end, int sublist_sizes[], int at, int pivots[], int first_p,
                            int last_p);
...
//Testing methods:
float omp_RecordTime(int *, int);

void omp_test(int,int);

/* sort an array in non-descending order */
//This function can be performed sequentially by running it with ONE thread
void psrs_sort(int *a, int n)
{
    if (n > 1)
    {
...
float omp_RecordTime(int *arr, int len)
{
...
#endif //REGULARSAMPLESORT_OMP_REGSAMPLESORT_H
